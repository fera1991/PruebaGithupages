...
export default function Login() {
    ...
    const onSubmit = async(data,e) =>{
        ...
        e.target.reset();
          ...
          const role = await context.login(data.user,data.password);
        if (role) {
        ...
        <div className="flex justify-center items-center min-h-screen bg-purple-50">
            <main className="bg-white max-w-lg p-8 md:p-12 my-10 rounded-sm shadow-2xl m-4">
                ...
                <form className="flex flex-col justify-center" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input className="font-sans block bg-gray-100 border w-full h-5 px-3 py-4 mt-2 hover:outline-none focus:outline-none focus:ring-1 focus:ring-gray-300 rounded-sm" type="password" placeholder="Password" type="password" {...register("password",{required:true})}>
                    </input>
